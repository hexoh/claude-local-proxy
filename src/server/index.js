...
async function handleMessages(req, res, cfg) {
  ...
  if (cfg.SERVER_KEY && !checkInboundAuth(req, cfg.SERVER_KEY)) {
  ...
  try {
    ...
    logger.logError(`[${reqID}] invalid inbound json: ${err.message}`);
    writeJSONError(res, 400, 'invalid_json');
    return;
  }

  if (!anthropicReq.model?.trim()) {
    logger.logError(`[${reqID}] missing model`);
  ...
  try {
    ...
    logger.logError(`[${reqID}] request conversion failed: ${err.message}`);
  ...
  if (anthropicReq.stream) {
    try {
      ...
      logger.logError(`[${reqID}] stream proxy error: ${err.message}`);
    }
    return;
  }
  ...
  try {
    ...
    logger.logInfo(`[${reqID}] upstream status=${upRes.statusCode}`);
    ...
    try {
      ...
      logger.logError(`[${reqID}] invalid upstream json: ${err.message}`);
    ...
    logger.logError(`[${reqID}] upstream request failed: ${err.message}`);
...
function createServer(cfg) {
  ...
  const server = http.createServer(async (req, res) => {
    logger.logAccess(`Received request: ${req.method} ${req.url}`);
